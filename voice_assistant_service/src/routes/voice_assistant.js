 ...
 */
 const fs = require('fs'); // for dummy function
 ...
 export async function dummySuccessProcess (client, sttResponse, request, humanEmotion, humanThayers, botThayers, grammarCorrectionPositions, grammarCorrectionPrediction, overlappedSpeech) {
  ...
  if (true) {
    ...
    // we introduce the voicebot response here
    let data
    console.log('Overlapped speech value: ', overlappedSpeech)
    if(overlappedSpeech) {
      await sleep(500)
      data = fs.readFileSync('./test_audio/goodbye.txt', 'utf8');
    }
    else {
      data = fs.readFileSync('./test_audio/welcome.txt', 'utf8');
    }
...
export async function dummyProcessAudioCommand (client, request) {
  ...
  const commandData = { text: 'User command' }

  await dummySuccessProcess(client, commandData, request, 'no detection for text-only command.', "n/a", "n/a", grammarCorrectionPositions, grammarCorrectionPrediction, request.audio.overlappedSpeech)
  // just added - END
}
 ...
 export async function dummyProcessTextCommand (client, request) {
  ...
  if (commandData.data === '') {
    ...
    var grammarCorrectionPositions = []
    var grammarCorrectionPrediction = ''
    await dummySuccessProcess(client, commandData, request, 'no detection for text-only command.', "n/a", "n/a", grammarCorrectionPositions, grammarCorrectionPrediction, false)
  }
}
 ...
 export async function processPossibleInterruption (client, interruptionClientConnection, request) {
  ...
  let botThayers="n/a"
  if (request.audio.type !== 'audio/wav' || request.audio.sampleRate !== 16000) {
    console.warn('Audio format is incorrect')
    // CAN REPLACE AGAIN WITH ORIGINAL
  } else {
    ...
    // make HTTP 2 request
    const req = interruptionClientConnection.request({
      ':method': 'POST',
      ':path': '/query-interrupt',
      'Content-Type': 'application/json',
    });

    // Prepare the body to send with the POST request
    const body = {
      id: request.id,
      audio: {
    ...
    req.write(JSON.stringify(body)); // add body to the request
    req.end(); // Signal that the request body has been fully written

    // Wait for the end event and process data then
    ...
    req.on('data', (chunk) => { dataReceived += chunk })

    // Once the response is finished, log the entire data
    // that we received

    // FOR THE MOMENT we don't close the connection after the request because we only open it once
    req.on('end', () => {
      const data = JSON.parse(dataReceived);
      if (data.isInterrupt) {
        console.log('SUCESS WE EMITTED THE INTERRUPTION !')
    ...
    req.on('error', (err) => {
      ...
      interruptionClientConnection.close(); // Close the connection in case of an error as well
    });
  }
}
catch (e){
